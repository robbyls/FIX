...
public class TestCaseNew implements ISocketAdaptorListener {
	...
	private static final Logger logger = LoggerFactory.getLogger(TestCaseNew.class);
	...
	{
		//register Test Case
		adaoptor.deRegisterListener(TestCaseNew.this);
	...
	private synchronized void runNext() {
		...
		if (messageList == null || messageList.size() == 0) {
			new Thread(new RunTestCase()).start();
		} else {
	...
	private class RunTestCase implements Runnable {
	...
	public void onMessage(IFIXMessage message) {
		...
		if (checkStopCondition(map)) {
			...
			logger.info("Abort the current run of the test case as received message" + APIClientUtils.printMapToString(map));
				...
				if (testCaseId.equals(requestId)) {
					...
					for (Map<String, String> stopCriteria : stopConditionMap) {
						...
						for (Map.Entry<String, String> oneCondition : stopCriteria.entrySet()) {
							...
							if (!value.equals(map.containsKey(key))) {
